...
class PositionalEncoding_Transition(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        super(PositionalEncoding_Transition, self).__init__()
...
class TRANSITION_TRANSFORMER_2code(nn.Module):
    def __init__(self, max_mov_len, device, latent_dim=512, ff_size=1024, num_layers=4, num_heads=4, dropout=0.1,
                 activation="gelu", **kargs):
        super().__init__()

        self.max_mov_len = max_mov_len
        self.device = device
        ...
        self.trans_token_0 = nn.Parameter(torch.randn(self.latent_dim))
        self.trans_token_1 = nn.Parameter(torch.randn(self.latent_dim))

        self.sequence_pos_encoder = PositionalEncoding_Transition(self.latent_dim, self.dropout)
    ...
    def forward(self, his_movements, his_real_mov_lens):
        bs, tmp_max_mov_len, mov_dim = his_movements.shape
        x = []
        x.append(his_movements)
        pad_movements = torch.zeros((bs, self.max_mov_len - tmp_max_mov_len, mov_dim), device=self.device)
        x.append(pad_movements)
        x = torch.cat(x, dim=1)
        mask = torch.zeros((bs, self.max_mov_len), dtype=bool, device=self.device)
        for i in range(bs):
            for j in range(min(tmp_max_mov_len, his_real_mov_lens[i])):
                mask[i][j] = True
        bs, max_mov_len, mov_dim = x.shape
        x = x.permute((1, 0, 2))  # (max_mov_len, bs, mov_dim)

        # adding the trans_token queries
        batch_trans_token_0 = self.trans_token_0.repeat(bs, 1)  # (bs, latent_dim)
        batch_trans_token_1 = self.trans_token_1.repeat(bs, 1)  # (bs, latent_dim)
        xseq = torch.cat((batch_trans_token_0[None], batch_trans_token_1[None], x), axis=0)

        # add positional encoding
        xseq = self.sequence_pos_encoder(xseq)

        # create a bigger mask, to allow attend to trans_token
        tokenMask = torch.ones((bs, 2), dtype=bool, device=self.device)
        maskseq = torch.cat((tokenMask, mask), axis=1)

        final = self.seqTransEncoder(xseq, src_key_padding_mask=~maskseq)
        trans_mov_0 = final[0]
        trans_mov_1 = final[1]

        return trans_mov_0, trans_mov_1
...
class TextEncoderCLIP(nn.Module):
    def __init__(self, hidden_size, device):
        ...
        self.input_emb = nn.Linear(512, hidden_size)
        self.input_emb.apply(init_weight)

    def forward(self, caption):  # caption:list(bs)
        text = clip.tokenize(caption).to(self.device)
        with torch.no_grad():
            text_features = self.clip_model.encode_text(text)
            features_norm = text_features / text_features.norm(dim=-1, keepdim=True)  # (bs, 512)
        clip_embs = self.input_emb(features_norm)  # (bs, 1024)
